#' Perform Portfolio Optimization and Visualization
#'
#' This function performs portfolio optimization on a given dataset and generates visualizations
#' for cumulative returns and turnover metrics.
#'
#' @param dat A pre-loaded data frame containing the dataset for analysis. The first column should contain the dates.
#' @param date_column_index An integer indicating the index of the date column in the data frame. Default is 1.
#' @param start_date A string representing the start date for filtering the data in 'YYYYMMDD' format. Default is '19990101'.
#' @param end_date A string representing the end date for filtering the data in 'YYYYMMDD' format. Default is '20231231'.
#' @param num_cores An integer specifying the number of CPU cores to use for parallel processing. Default is 7.
#'
#' @return A list containing cumulative return and turnover plots, as well as other calculated metrics.
#' @import glmnet quadprog doParallel mvtnorm doSNOW lubridate CCA Matrix tictoc corpcor PerformanceAnalytics ggplot2 reshape2
#' @export
#'
#' @examples
#' \dontrun{
...
#' result <- perform_portfolio_optimization(dat = data, date_column_index = 1,
#'                                          start_date = '19990101', end_date = '20231231', num_cores = 7)
#' # Access the plots
#' print(result$cumulative_return_plot)
#' print(result$cumulative_turnover_plot)
#' }
perform_portfolio_optimization <- function(dat, date_column_index = 1, start_date = '19990101', end_date = '20231231', num_cores = 7) {
  ...
  # Prepare data
  dat <- dat[,-date_column_index] # Remove the date column
  ...
  for (i in (7:max(mon))) {
    ...
    # w.tmp[,8] <- po.covShrink(rep(0, n_tmp), x_tmp)                                          # LW
